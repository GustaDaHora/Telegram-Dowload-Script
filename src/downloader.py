...
def get_filename(message):
    """Helper to compute the filename for the media without downloading."""
    if message.photo:
...
async def download_file(message, download_path, progress_bars):
    try:
        ...
        if os.path.exists(full_path):
            print(f"{Fore.YELLOW}Skipping existing file: {filename}{Style.RESET_ALL}")
            return
...
async def download_in_batches(messages, download_path, batch_size):
    progress_bars = []  # Shared list for all progress bars (positions accumulate)
    tasks = []
    for i, message in enumerate(messages):
        tasks.append(download_file(message, download_path, progress_bars))
...
async def main():
    try:
        async with TelegramClient(session_name, api_id, api_hash) as client:
            ...
            if media_messages:
                await download_in_batches(media_messages, download_path, batch_size)
            else:
